...
export const systemNodes = [
  {
    id: 'borgella',
    label: 'Borgella Gateway',
    subtitle: 'VPS • Claude Opus 4',
    status: 'online',
    ...
    children: [
      ...
      { id: 'oc-slack', label: 'Slack Agent (borgella-slack)', status: 'online', tooltip: 'Secondary agent instance operating in the Measurelab Slack workspace (The Bridge). Isolated sessions with scoped access — can only reference system wiki, Measurelab Toolkit, and channel content. Never exposes personal vault or memory. Can communicate with main agent via sessions_send for cross-channel coordination.' },
      ...
      { id: 'oc-orchestrate', label: 'Orchestration Pipeline', status: 'online', tooltip: 'Concept-to-launch pipeline using Claude Code CLI. Borgella picks tasks from the kanban, invokes Claude Code on the VPS to build/execute, then moves tasks through QA. Triggered on demand via batch skill.' },
      { id: 'oc-alerts', label: 'Alerting System', status: 'online', tooltip: 'Systemd timer runs every 30 minutes checking: service health (Borgella, voice bridge), stuck kanban tasks (>24h in progress), disk space (>80%). Sends alerts via Telegram. Deduplicates within 6 hours.' },
  ...
  },
  {
    ...
    subtitle: 'VPS :8100 • Candidate for retirement',
    status: 'paused',
    tooltip: 'Multi-agent orchestration layer. Candidate for retirement — Borgella sub-agent orchestration handles the same use cases more effectively. Still running but not actively used.',
    icon: '🔀',
  ...
  },
  {
    ...
    status: 'online',
    tooltip: 'The underlying VPS infrastructure that hosts Borgella and supporting services. Secured behind Tailscale mesh network with UFW firewall rules.',
    icon: '🖥️',
    ...
    children: [
      ...
      { id: 'vps-openai', label: 'OpenAI API (embeddings)', status: 'online', tooltip: 'OpenAI text-embedding-3-small for vector embeddings used by Memindex. GPT-4o-mini for chunk enrichment (topic tags, entity extraction, semantic expansion). Replaced local Ollama on Feb 9.' },
      ...
      { id: 'vps-voicebridge', label: 'Voice Bridge (port 5052)', status: 'paused', tooltip: 'Fastify relay server connecting Gemini Live voice chat to Borgella. Currently parked — feedback loop needs redesign. Systemd service exists but not actively used.' },
  ...
  },
  {
    ...
    children: [
      ...
      { id: 'mi-lancedb', label: 'LanceDB + OpenAI Embeddings', status: 'online', tooltip: 'LanceDB vector database with OpenAI text-embedding-3-small embeddings (1536 dims). Replaced local Ollama on Feb 9 for better quality.' },
      { id: 'mi-enrichment', label: 'LLM Enrichment (GPT-4o-mini)', status: 'online', tooltip: 'Each chunk gets enriched with topic tags, entity extraction, and semantic expansion via GPT-4o-mini before embedding. Bridges the semantic gap between query terms and source vocabulary. Cost: ~$1.59 for full index.' },
      { id: 'mi-search', label: 'Vector Search + Query Expansion', status: 'online', tooltip: 'Cosine similarity search on enriched embeddings. When confidence < 0.5, GPT-4o-mini generates 3-5 alternative queries for broader matching.' },
      { id: 'mi-sources', label: 'Sources (~11,500 chunks)', status: 'online', tooltip: 'Indexed content: vault (journals with sentence-level chunking, notes, references), session logs (active + deleted), kanban tasks, Measurelab toolkit. Auto-synced every 6h.' },
  ...
  },
  {
    ...
    status: 'paused',
    tooltip: 'Real-time voice conversation app using Google\'s Gemini Live API (native audio model). Deployed on Vercel. Supports bidirectional voice streaming — like a phone call with AI. Connected to Borgella via the Voice Bridge relay.',
    icon: '🎙️',
    position: { x: 30, y: 450 },
    children: [
      ...
      { id: 'vc-bridge', label: 'Voice Bridge Relay', status: 'online', tooltip: 'Fastify server on VPS port 5052 (systemd service). Receives function calls from Gemini, forwards to Telegram as commands. Supports feedback loop: Borgella posts results back, app polls and feeds them to Gemini as tool responses.' },
      { id: 'vc-feedback', label: 'Feedback Loop', status: 'online', tooltip: 'Full round-trip: Gemini calls function → relay forwards to Borgella → Borgella processes → posts result to relay /result endpoint → app polls every 3s → sends tool response to Gemini → Gemini speaks the result.' },
    ]
  },
];
...
export const systemEdges = [
  ...
  { id: 'e-oc-mi', source: 'borgella', target: 'memindex', label: 'Search', tooltip: 'Borgella queries Memindex to find relevant context from across all knowledge sources. Semantic search helps the agent recall information it doesn\'t have in active memory.' },
  ...
  { id: 'e-openai-mi', source: 'openai-ext', target: 'memindex', label: 'Embeddings + Enrichment', tooltip: 'OpenAI API provides embeddings (text-embedding-3-small) and LLM enrichment (GPT-4o-mini) for Memindex chunks. Replaced local Ollama on Feb 9.' },
  { id: 'e-oc-vault', source: 'borgella', target: 'vault', label: 'File access', tooltip: 'Borgella reads and writes Obsidian vault markdown files directly on the VPS filesystem. Used for memory updates, note creation, and knowledge base queries.' },
  ...
  { id: 'e-oc-anthropic', source: 'borgella', target: 'anthropic-ext', label: 'LLM calls', tooltip: 'All Claude model inference runs through the Anthropic API. Every message, tool use, and reasoning step is an API call to Anthropic\'s servers.', animated: true },
  { id: 'e-vc-bridge', source: 'voicechat', target: 'vps', label: 'Function calls', tooltip: 'Gemini Live fires function calls to the Voice Bridge relay server on VPS port 5052. The relay forwards these as Telegram messages to Borgella for execution. Async fire-and-forget model.' },
  ...
  { id: 'e-bridge-telegram', source: 'vps', target: 'telegram-ext', label: 'Voice bridge relay', tooltip: 'The Voice Bridge relay on VPS sends Telegram messages to Borgella on behalf of the voice chat. Borgella processes the command and responds via Telegram as normal.' },
  { id: 'e-oc-claude', source: 'borgella', target: 'vps', label: 'Claude Code CLI', tooltip: 'The orchestration pipeline invokes Claude Code CLI on the VPS to autonomously build and execute tasks. Uses a PTY wrapper (script -qec) for proper terminal emulation.' },
];
...
export const externalNodes = [
  ...
  { id: 'anthropic-ext', label: 'Anthropic API', icon: '🤖', position: { x: 1350, y: 370 }, tooltip: 'Anthropic\'s Claude API — the LLM provider powering Borgella. All reasoning, code generation, and conversation runs through this API. Critical external dependency.' },
  ...
  { id: 'openai-ext', label: 'OpenAI API', icon: '🧪', position: { x: 1350, y: 520 }, tooltip: 'OpenAI API for Memindex: text-embedding-3-small (embeddings) and GPT-4o-mini (chunk enrichment + query expansion). ~$0.74/month typical usage.' },
];
